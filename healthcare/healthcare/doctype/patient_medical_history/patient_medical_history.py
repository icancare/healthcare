...
class PatientMedicalHistory(Document):
	...
	def validate_since_when_format(self):
		"""
		Validate Since When field format
		...
		if match_year:
			...
			if year > current_year:
				frappe.throw(_("Year in 'Since When' field cannot be greater than current year ({0})").format(current_year))
		elif match_month_year:
			month = int(match_month_year.group(1))
			year = int(match_month_year.group(2))
			
			if month < 1 or month > 12:
				frappe.throw(_("Month in 'Since When' field must be between 1 and 12"))
			
			if year > current_year:
				frappe.throw(_("Year in 'Since When' field cannot be greater than current year ({0})").format(current_year))
		else:
			frappe.throw(_("'Since When' field must be in format YYYY (e.g., 2023) or MM/YYYY (e.g., 01/2023, 1/2023)"))
